...
const {addUser, removeUser, getUser, getUsersInRoom, startGame, getGameState, movePiece, updateBoardState} = require('./users');
...
io.on('connection', (socket) => {
    ...
    socket.on('join', ({room, random}, callback) => {
        ...
        socket.emit('connected', {player: user.name, room: user.room, boardState: getGameState(user.room)});
        socket.emit('message', {user: 'admin', text: 'Welcome to the game!'});
        socket.broadcast.to(user.room).emit('message', {user: 'admin', text: `User ${user.name} has joined the game!`});
